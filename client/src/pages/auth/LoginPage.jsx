...
const LoginPage = () => {
  ...
  const { loginByEmail, isLoggingIn, googleLogin, isGoogleLoggingIn, facebookLogin, isFacebookLoggingIn } = useAuthStore()
      ...
      <div className="w-full lg:w-[45%] h-full flex flex-col items-center justify-center px-8 relative overflow-y-auto" style={{ background: '#080808' }}>
        ...
        <motion.div className="relative z-10 w-full max-w-[360px] flex flex-col gap-6" variants={formContainer} initial="hidden" animate="visible">
          ...
          <motion.div className="text-center" variants={formItem}>
            <h1 className="text-[24px] font-bold text-ghost tracking-tight mb-1">Welcome back</h1>
            <p className="text-[13px] text-dim">Log in and keep your content running.</p>
          </motion.div>
              ...
              <motion.form key="email" onSubmit={handleSubmit} className="flex flex-col gap-3" variants={modeSwitch} initial="hidden" animate="visible" exit="exit">
                ...
                <div className="flex flex-col gap-1">
                  <div className="flex justify-between">
                    ...
                    <Link to="/forgot-password" size="small" className="text-[11px] text-dim hover:text-ghost transition-colors">Forgot?</Link>
                  </div>
                  <div className="relative">
                    <input type={show ? 'text' : 'password'} name="password" value={form.password} onChange={handleChange} placeholder="••••••••" required className={`${inputCls} pr-14`} style={inputStyle} onFocus={e => e.target.style.borderColor = 'rgba(57,255,20,0.5)'} onBlur={e => e.target.style.borderColor = '#2A2A2A'} />
                ...
                <motion.button type="submit" disabled={isLoggingIn} className="btn-neon w-full py-3 text-[13.5px] font-bold rounded-[8px] mt-1 disabled:opacity-50 disabled:cursor-not-allowed" whileHover={{ scale: isLoggingIn ? 1 : 1.02 }} whileTap={{ scale: isLoggingIn ? 1 : 0.97 }}>
          ...
          <motion.p className="text-center text-[12px] text-dim" variants={formItem}>
            No account?{' '}
            <Link to="/signup" style={{ color: '#39FF14' }} className="font-semibold hover:underline">Sign up free</Link>
          </motion.p>
